...
export function SettingsPanel({ isOpen, onClose, onConfigChange, aiService }: SettingsPanelProps) {
  ...
  const handleTest = async () => {
    ...
    try {
      ...
      setTestStatus('error');
      setTestMsg(err instanceof AiServiceError ? err.message : '连接失败');
    }
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
      <div className="w-96 rounded-2xl border border-white/10 bg-[#1a1a2e] shadow-2xl">
        <div className="flex items-center justify-between px-5 py-4 border-b border-white/10">
          ...
          <button
            onClick={onClose}
            className="text-white/30 hover:text-white/70 transition-colors text-sm leading-none px-1"
          >✕</button>
        </div>
        ...
        <div className="p-5 space-y-4">
          <div>
            ...
            <input
              ...
              className="w-full px-3 py-2 rounded-lg bg-white/5 border border-white/10
                         ...
                         focus:outline-none focus:border-red-400/40 transition-colors"
            />
          </div>
          ...
          <div className="flex gap-3 pt-1">
            <button
              ...
              className="flex-1 px-4 py-2 rounded-lg border border-white/10 text-white/60 text-sm
                         hover:bg-white/5 hover:text-white/80 disabled:opacity-40 transition-all"
            >
            ...
            <button
              ...
              className="flex-1 px-4 py-2 rounded-lg bg-red-500 text-white text-sm
                         hover:bg-red-400 active:scale-95 transition-all"
            >
              保存
            </button>
          </div>
